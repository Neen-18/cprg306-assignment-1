...
function displayCharacterNames(characterArray, listId) {
...
displayCharacterNames(characters, "function-list");
...
function displayCharacterFilteredAge(characterArray, ageThreshold, listId) {
  const list = document.getElementById(listId);

  const ageFilteredCharacters = characterArray.filter(character => character.age < ageThreshold);

  ageFilteredCharacters.forEach(character => {
    const li = document.createElement("li");
    li.textContent = character.name;
    list.appendChild(li);
  });
...
displayCharacterFilteredAge(characters, 40, "age-filter-list");
...
function logAndDisplayError(message, errorDivId) {
...
function displayCharacterNamesSafe(characterArray, listId, errorDivId) {
  const list = document.getElementById(listId);
...
function displayCharacterFilteredAgeSafe(characterArray, ageThreshold, listId, errorDivId) {
  const list = document.getElementById(listId);
...
displayCharacterNamesSafe(characters, "error-handling-list", "error-messages");
displayCharacterFilteredAgeSafe(characters, 40,  "error-handling-list", "error-messages");
...
displayCharacterNamesSafe(brokenCharacters, "broken-array-list", "broken-array-errors");